...
import SlashCommand, { SlashCommandResolvable, SlashCommandOptions as ApplicationCommandData } from './SlashCommand.js';
...
class ApplicationCommandManager {
    ...
    public async post(command: SlashCommandResolvable): Promise<ApplicationCommand | null> {
        if (!this.client.application)
            throw new Error('The bot is not yet logged in: run this method in the client\'s \'ready\' event.');

        if (!(command instanceof SlashCommand))
            return this.post(new SlashCommand(command));
    ...
    public async edit(command: ApplicationCommand, data: ApplicationCommandData): Promise<ApplicationCommand | null> {
        if (!this.client.application)
            throw new Error('The bot is not yet logged in: run this method in the client\'s \'ready\' event.');
        ...
        const editted = await manager.edit(command.id, new SlashCommand({ ...command, ...data }).toAPIObject());

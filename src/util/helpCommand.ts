    ...
    static get help() {
        ...
        return new Command()
            ...
            .setCallback(function (message, args, client) {
                const input = toString(args.first()?.value).toLowerCase();
                ...
                if (group) {
                    const commands = client.manager.index.array().filter(command => command.group === group).map(command => {
                        const field: EmbedFieldData = { name: `${client.manager.prefix}${command.name} ${command.parameters.array().length > 0 ? command.parameters.array().map(i => `\`${i.label}${!i.required ? '?' : ''}\``).join(' ') : ''}`, value: command.description || 'No description', inline: false };
